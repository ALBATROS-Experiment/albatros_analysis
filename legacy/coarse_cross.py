...
if __name__=="__main__":
    ...
    for fname in fnames:
        ...
        ntime = np.shape(data['pol0'])[0]
        nchan = np.shape(data['pol0'])[1]
        ...
        if opts.dsfac is True:
            ...
            mean_pol00 = downsample2d(np.abs(data['pol0'])**2, opts.dsfac)
            mean_pol11 = downsample2d(np.abs(data['pol1'])**2, opts.dsfac)
        ...
        print(f'Total dimensions are now {np.shape(pol01_mag)}')
    ...
    myext = np.array([fmin, fmax, np.shape(pol01_mag)[0], 0])
    pylab.subplot(2,2,1)
