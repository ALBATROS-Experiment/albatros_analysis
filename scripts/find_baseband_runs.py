"""A script that trolls through logs directory and prints contents of log files between two specified dates. Needs a directory that contains the following folders:
...
def chan2freq(chan):
    """Helper function to the script?? Should we hard code 2048??"""
    return np.round(125.0 * chan / 2048.0, 2)
...
def logs2ctimes(logdir):
    ...
    for fname in fnames:
        ...
        ctime = int(
            (
                datetime.datetime.strptime(tstamp, "%d%m%Y_%H%M%S")
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        dest="logdir",
        type="str",
        default="/project/s/sievers/simont/logs",
    ...
    if opts.events_file != "None":
        ...
        for line in fp.readlines():
            ...
            events_ctimes.append(
                int(
                    (
                        datetime.datetime.strptime(tstamp, "%Y%m%d_%H%M%S")
                        - datetime.datetime(1970, 1, 1)
                    ).total_seconds()
                )
            )
    ...
    config_logfiles, config_logfiles_ctimes = logs2ctimes(
        opts.logdir + "/albatros_config_fpga"
    )
    baseband_logfiles, baseband_logfiles_ctimes = logs2ctimes(
        opts.logdir + "/albatros_dump_baseband"
    )
    ...
    if opts.start is not None:
        ctstart = int(
            (
                datetime.datetime.strptime(opts.start, "%Y%m%d")
                - datetime.datetime(1970, 1, 1)
            ).total_seconds()
        )
    if opts.stop is not None:
        ...
        ctstop = (
            int(
                (
                    datetime.datetime.strptime(opts.stop, "%Y%m%d")
                    - datetime.datetime(1970, 1, 1)
                ).total_seconds()
            )
            + 86399
    ...
    inds = np.where(
        (baseband_logfiles_ctimes >= ctstart) & (baseband_logfiles_ctimes <= ctstop)
    )[0]
    if len(inds) == 0:
    ...
    for baseband_logfile in baseband_logfiles:
        ...
        ctstart = int(
            (
                datetime.datetime.strptime(tstart, "%d-%m-%Y %H:%M:%S")
                - datetime.datetime(1970, 1, 1)
            ).total_seconds()
        )
        ...
        ctstop = int(
            (
                datetime.datetime.strptime(tstop, "%d-%m-%Y %H:%M:%S")
                - datetime.datetime(1970, 1, 1)
            ).total_seconds()
        )
    ...
    for ind in range(len(baseband_logfiles))[-1::-1]:
        ...
        if opts.bits is not None:
            s = re.compile(r"(\d)$")
            btxt = os.popen(
                'grep "Baseband bits" ' + baseband_logfiles[ind] + ' | cut -d " " -f6-'
            ).readlines()[0]
            if s.search(btxt):
        ...
        print(
            "Start time:",
            str(datetime.datetime.utcfromtimestamp(baseband_logfiles_ctimes[ind])),
        )
        ...
        print(btxt.strip())
        ctxt = os.popen(
            'grep "Channels" ' + baseband_logfiles[ind] + ' | cut -d " " -f6-'
        ).readlines()[0]
        print(ctxt.strip())
        if config_logfile is None:
            ...
            cctxt = os.popen(
                'grep "Channel coeffs" ' + config_logfile + ' | cut -d " " -f6-'
            ).readlines()[0]
            print(cctxt.strip())
        s = re.compile(r"(\d+)\:(\d+)$")
        if s.search(ctxt):
        ...
        if config_logfile is None:
            ...
            atxt = os.popen(
                'grep "ADC bits" ' + config_logfile + ' | cut -d " " -f4-'
            ).readlines()
            if len(atxt) > 0:
                print(atxt[0].strip())
            else:
        ...
        if config_logfile is not None:
            ftxt = os.popen(
                'grep "ADC initialisation failed" ' + config_logfile + " | wc -l"
            ).readlines()[0]
