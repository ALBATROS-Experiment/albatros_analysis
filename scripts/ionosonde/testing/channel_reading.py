...
aa2 = bdc.BasebandFileIterator(
    files[0],
    0,  # fileidx is 0 = start idx is inside the first file
    idxs[0],
    read_size,
    nchunks=nchunks,
    channels=channel_indices2,
    type="float",
)
...
final_channels2 = aa2.obj.channels[aa2.obj.channel_idxs].copy()

print("Antenna object channels 1:", aa1.obj.channels)
print("Final channels 1:", final_channels1)

print("Antenna object channels 2:", aa2.obj.channels)
print("Final channels 2:", final_channels2)

nchan1 = len(final_channels1)
nchan2 = len(final_channels2)

chunk_idx = 0
chunks1 = zip(*antenna_objs1).__next__()
chunks2 = zip(*antenna_objs2).__next__()

ant_idx = 0
chunk1 = chunks1[ant_idx]
chunk2 = chunks2[ant_idx]


print(chunk1["pol0"])
print(chunk2["pol0"])

print(np.sum(chunk1["pol0"][:,20:40] != chunk2["pol0"]))