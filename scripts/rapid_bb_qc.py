if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=100000,
        help="Accumulation length for averaging. If bigger then number of spectra, only one output row",
    )
    parser.add_argument(
        "-r",
        "--readlen",
    ...
    parser.add_argument(
        ...
        type=int,
        nargs=2,
        help="Indices of start and end channels. Start channel index MUST be even in case of 1 bit file.",
    )

    args = parser.parse_args()
    if not args.chans:
        args.chans = [0, None]
    obj = bdc.BasebandPacked(
        args.fpath, readlen=args.readlen, chanstart=args.chans[0], chanend=args.chans[1]
    )
    ...
    if obj.bit_mode == 4:
        ...
        for i in range(0, nchunks):
            ...
            pol01[i, :] = cr.avg_xcorr_1bit(
                obj.pol0[st:en], obj.pol1[st:en], specnum, nchans
            )
