...
def repfb_xcorr_avg(idxs,files,pfb_size,nchunks,channels,osamp,new_acclen,outfile,lblock=4096, ntap=4, cutsize=16,filt_thresh=0.45):
    ...
    if rowidx>0:
        ...
        print('total pointer + rowidx', vis_file_ptr +rowidx)
...
def repfb_xcorr_avg_part(idxs,files,pfb_size,nchunks,channels,osamp,new_acclen,outfile,lblock=4096, ntap=4, cutsize=16,filt_thresh=0.45):
    """Re-PFB baseband spectra for all antennas x polarizations and x-corr all frequencies
    ...
    if rowidx>0:
        ...
        print('vis file row rowidx+1', vis_file[0, rowidx+1, 0, 0])
        print('vis file row rowidx', vis_file[0, rowidx, 0, 0])
        print('vis file rowidx -1', vis_file[0, rowidx-1, 0, 0])
        print("writing", fname)
        np.save(fname, vis_file)
    print("final rowidx=", rowidx)
    print('total seen rows', total_seen_rows)
    print('net total:', (vis_chunk_id * vis_chunk_size) +rowidx)
    print('total rows we should have seen', nrows_total)
