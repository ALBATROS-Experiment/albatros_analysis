...
ant=bdc.BasebandFileIterator(files, start_file_idx, 0, acclen, nchunks=nchunks, type='float')
to_ipfb = cp.empty((pfb_size,nchans),dtype='complex64', order='C')
print("to_ipfb size", np.prod(to_ipfb.shape)*8/1024**3, "GB")
...
for i, chunk in enumerate(ant):
    ...
    if i==0:
        ...
        pol0=bdc.make_continuous_gpu(chunk['pol0'],chunk['specnums']-chunk['specnums'][0],channels,acclen,nchans=2049)
        to_ipfb[:2*cut,:] = pol0[-2*cut:,:].copy() # the only problem is if it's all zeros, first cut rows of next block will be zeros
        continue
    if missing>0.1:
    ...
    to_ipfb[2*cut:] = pol0.copy()
    raw_pol0 = pu.cupy_ipfb(to_ipfb, matft, thresh=filt_thresh)
    print("IPFB done")
    pol0_new = pu.cupy_pfb(raw_pol0[cut:-cut],cupy_win_big,nchan=2048*osamp+1,ntap=4)
    print("pfb done")
    to_ipfb[:2*cut] = pol0[-2*cut:].copy() #store for next iteration
    # end_event.record()
    # end_event.synchronize()
    # print("cupy total time taken to do ipfb/pfb",cp.cuda.get_elapsed_time(start_event, end_event)/1000)
    # do stuff with new pol0
    # start_event.record()
    avg_data[i-1,:] = cp.asnumpy(cp.mean(pol0_new*cp.conj(pol0_new),axis=0)[start_chan:end_chan])
    # end_event.record()
    # end_event.synchronize()
