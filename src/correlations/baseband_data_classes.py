...
class Baseband:
    def __init__(self, file_name, readlen=-1, force_cpu=False):
...
class BasebandFloat(Baseband):
    def __init__(
        ...
        force_cpu: bool, optional
            If True, raw_data will be stored on CPU (NumPy) even if GPU (CuPy) is available.
        """
        super().__init__(file_name, readlen, force_cpu=force_cpu)
...
class BasebandPacked(Baseband):
    ...
    def __init__(
        ...
        super().__init__(file_name, readlen, force_cpu=force_cpu)  # why not pass fixoverflow too??
        self._assign_channels(channels=channels,chanstart=chanstart,chanend=chanend)
