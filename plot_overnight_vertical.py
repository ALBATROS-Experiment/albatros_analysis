...
def full_plot(data_arrs, mytz, chunk_time):
    ...
    plt.figure(figsize=(13,18), dpi=200)
    plt.subplot(3,2,1)
    ...
    outfile = os.path.join(outdir,'spectra_overview_' + str(ctime_start) + '_' + str(ctime_stop) + '.jpg')
    plt.savefig(outfile)
...
def main():
    ...
    if vmin is None and vmax is None:
        ...
        vmin, _ = get_vmin_vmax(pol00)
        vmin2, _ = get_vmin_vmax(pol11)
        vmax2 = vmax
    elif vmax is None: # and vmin is not None
        _, vmax = get_vmin_vmax(pol00)
        _, vmax2 = get_vmin_vmax(pol11)
        vmin2 = vmin
    else: # vmin and vmax are both not None
        vmin2 = vmin
        vmax2 = vmax
    
    if logplot:
        if vmin > 1e2: # allows for easy manual input of vmin in logspace, ex --vmin 6.7 sets vmin=10**6.7
            vmin = np.log10(vmin)
