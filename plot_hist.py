...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "filepath",
        type=str,
        help="Baseband file locaion. Ex: ~/snap1/16171/161700000/161700026.raw",
    )
    parser.add_argument(
        "-o", "--output_dir", type=str, default="./", help="Output directory for plots"
    )
    parser.add_argument(
        "-m", "--mode", type=int, default=-1, help="0 for pol0, 1 for pol1, -1 for both"
    )
    parser.add_argument(
        "-r",
        "--rescale",
        action="store_true",
        help="Map bit values (0-15 for 4 bit data) to -ve to +ve levels.",
    )
    parser.add_argument(
        "-c",
        "--chans",
        type=int,
        nargs=2,
        help="Indices of start and end channels to print out.",
    )
    ...
    if args.rescale and obj.bit_mode == 4:
        bins = np.arange(-7, 8)
        hist = np.fft.fftshift(
            hist, axes=0
        )  # first row would correspond to -8 which is 0
