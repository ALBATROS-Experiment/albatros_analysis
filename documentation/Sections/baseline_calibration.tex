...
we will obtain complex visibilities with the geometric time delay $\tau_g$ encoded into them. This is derived below:
\begin{eqnarray}
...
\end{eqnarray}
Here we assume that the satellite moves slowly with respect to the integration time. 
Since $E_0$ correlated with itself is high, we expect a dominant presence of the $e^{-2 \pi i \nu_0 \tau_g}$ term
even in a realistic, noisy sky.
...
To obtain a reasonably good plot, we must often massage the data into a better form. 
For example, we interpolate over missing chunks (directly in the visibilities), and we must also sometimes 
cut the pulse from the front or the back to remove noisy or aliased (check if this is actually aliasing when going fast, 
but either way) parts.
...
\begin{itemize} 
    \item there is some data missing, happens far more often with 1bit data. we interpolate over bad data directly in the visibility. done in rectangular and then recombined. show an example plot
    \item we must then also filter through the bad UNWRAPPED visibilities phases. the current model fits for unwrapped phases, so want to ensure we have a continuous and clear unwrapped phase plot.
    \item in the future, implement one of, or both of, automatic slicing and interpolation over bad data. This would speed up the data pipeline and make more pulses usable for a longer time.
\end{itemize}
...
\begin{figure}[H]
  ...
  \begin{minipage}{0.45\textwidth}
    ...
    \caption{Unfitted predicted phase against observed phase. Visible discrepancy.}
    \label{fig:unfitted1}
  \end{minipage}
  \hfill
  \begin{minipage}{0.45\textwidth}
    ...
    \caption{Different predicted phase, now plotted alongside observed phase. More significant discrepancies.}
    \label{fig:unfitted2}
  \end{minipage}
\end{figure}


Some pulses will initially be rather well fit, such as Figure~\ref{fig:unfitted1}, whereas others will leave more to be desired, such as Figure~\ref{fig:unfitted2}. These illustrate the necessity of fitting for multiple satellite passes simultaneously, as different satellites will have different trajectories in the sky, and thus will favor different fits.
...
\begin{figure}[H]
  ...
  \begin{minipage}{0.45\textwidth}
    \centering
    \includegraphics[width=\linewidth]{Figures/bad_pulse_phase1.jpeg}
    \caption{Unfitted predicted phase against observed phase. Visible discrepancy.}
    \label{fig:bad_pulse_phase1}
  \end{minipage}
  \hfill
  \begin{minipage}{0.45\textwidth}
    \centering
