...
def get_binned(fname, data_dir, coords, ctime_start, ctime_stop, nbins):
    ...
    for i, pol00 in enumerate(datpol00):
        ...
        for i in range(len(branch_points)):
            ...
            binned[lstbins[b]] = np.vstack(
                [binned[lstbins[b]], pol00[st:en, :]]
            )  # prealocated arrays would prolly speeden this up
            st = en
    return binned
...
if __name__ == "__main__":
    ...
    plt.title("Pol00")
    plt.imshow(
        np.log10(statsp00[plot_type]), vmin=7, vmax=8.2, extent=myext, aspect="auto"
    )
    plt.colorbar()
    ...
    plt.title("Pol11")
    plt.imshow(
        np.log10(statsp11[plot_type]), vmin=7, vmax=8.2, extent=myext, aspect="auto"
    )
    plt.colorbar()
    ...
    output_path = (
        f"/project/s/sievers/mohanagr/lst_{nbins}_{ctime_start}_{ctime_stop}_{loc}.npz"
    )
    np.savez_compressed(
        output_path,
        p00mean=statsp00["mean"],
        p00median=statsp00["median"],
        p11mean=statsp11["mean"],
        p11median=statsp11["median"],
        p01rmean=statsp01r["mean"],
        p01rmedian=statsp01r["median"],
        p01imean=statsp01i["mean"],
        p01imedian=statsp01i["median"],
