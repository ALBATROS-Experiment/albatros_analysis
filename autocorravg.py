...
def get_avg_fast(path, init_t, end_t, acclen, nchunks, chanstart=0, chanend=None):
    ...
    try:
        for i, chunk in enumerate(ant1):
            ...
            pol01[i, :] = cr.avg_xcorr_4bit(
                chunk["pol0"], chunk["pol1"], chunk["specnums"]
            )
            t2 = time.time()
            print("time taken for one loop", t2 - t1)
            j = ant1.spec_num_start
            print(
                "After a loop spec_num start at:",
                j,
                "Expected at",
                m + (i + 1) * acclen,
            )
    ...
    coeffs = getattr(ant1.obj, "coeffs", None)
    if coeffs is None:
        print("No obj.coeffs found")
        data = (pol00, pol11, pol01, ant1.obj.channels)
    else:
        data = (pol00, pol11, pol01, ant1.obj.channels, coeffs)
...
if __name__ == "__main__":
    ...
    if len(result) == 5:
        pol00, pol11, pol01, channels, coeffs = result
        np.savez_compressed(
            fpath,
            datap01=pol01.data,
            maskp01=pol01.mask,
        ...
        pol00, pol11, pol01, channels = result
        np.savez_compressed(
            fpath,
            datap01=pol01.data,
            maskp01=pol01.mask,
    ...
    butils.plot_4bit(
        ...
        pol11,
        pol01,
        channels[args.chans[0] : args.chans[1]],
        args.acclen,
        args.time_start,
        args.vmin,
        args.vmax,
