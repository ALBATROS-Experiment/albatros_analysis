...
def write_header(file_object, chans, spec_per_packet, bytes_per_packet, bits):
    have_trimble = True
    header_bytes = 8 * 10 + 8 * len(chans)  # 8 bytes per element in the header. Should be 11.
...
def dump_raw_data():
    ...
    print(packet)
    with open("/home/mohan/Projects/albatros_analysis/correlations/tests/header.raw", "w") as f:
