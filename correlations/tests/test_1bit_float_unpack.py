...
def expected_complex_1bit(real_im_1bit_pol0, real_im_1bit_pol1):
    ...
    return real_im_1bit_pol0[0]+1J*real_im_1bit_pol0[1],real_im_1bit_pol1[0]+1J*real_im_1bit_pol1[1] 
...
def packed_1bit(real_im_1bit_pol0, real_im_1bit_pol1):
    ...
    for i in range(0,2):
        for j in range(0,4):
            packed[0,i*4+j] = (p0r[i,2*j]<<7)+(p0im[i,2*j]<<6)+(p1r[i,2*j]<<5)+(p1im[i,2*j]<<4)+\
                            (p0r[i,2*j+1]<<3)+(p0im[i,2*j+1]<<2)+(p1r[i,2*j+1]<<1)+(p1im[i,2*j+1])
    return packed
...
def test_unpack(packed_1bit,real_im_1bit_pol0, real_im_1bit_pol1):
    ...
    assert pol0[1,0] == str_to_bits('01110101') #channel 3 the different one is in first byte
