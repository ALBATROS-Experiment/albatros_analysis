...
libpath=os.path.join(os.path.dirname(os.path.abspath(__file__)),'libalbatrostools.so')
...
def get_data_raw(file_name, items=-1,byte_delta=-8,fill_gaps=False):
    ...
    if fill_gaps:
        ...
        pol0=numpy.zeros([nn,nchan],dtype='uint8')   #in new code I represent everything as uint8 -Mohan
