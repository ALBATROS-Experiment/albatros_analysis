...
def unpack_packet(packet,bits,spec_per_packet):
    if bits==4:
        vec=unpack_4bit(packet) #unpack_4bit(packet[4:]) first 4 bytes were specnum, which I dont have here
...
def expected_complex_4bit(real_im_4bit_pol0, real_im_4bit_pol1):
    ...
    return real_im_4bit_pol0[0]+1J*real_im_4bit_pol0[1],real_im_4bit_pol1[0]+1J*real_im_4bit_pol1[1] 
...
def packed_4bit(real_im_4bit_pol0, real_im_4bit_pol1):
    ...
    pshape = packed_pol0.shape[0]*packed_pol0.shape[1] + packed_pol1.shape[0]*packed_pol1.shape[1]
    packed=np.zeros(pshape,dtype='uint8')
    packed[0::2]=np.ravel(packed_pol0)
    packed[1::2]=np.ravel(packed_pol1)
    return packed
...
#     pol0,pol1=unpk.unpack_4bit(packed_4bit,10,0,2,0,10)
#     truepol0,truepol1 = unpack_packet(packed_4bit,4,2)
