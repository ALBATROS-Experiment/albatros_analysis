...
def get_init_info(init_t, end_t, parent_dir):
    ...
    ----------
    init_t: int or float
        Start timestamp of time we are interested in. (in what units??)
    end_t: int or float
        End of time window we are interested in. (in what units??)
    parent_dir: str
...
def plot_4bit(
    ...
    if (vmin is None) and (vmax is None):
        med, vmin, vmax = get_plot_lims(pol00, acclen) # use of acclen in get_plot_lims is depricated
        med2, vmin2, vmax2 = get_plot_lims(pol11, acclen) # use of acclen in get_plot_lims is depricated
    else:
